...
def main():
    """
    code to show a specific highlight for a specific agent.
    ...
    data.
    usage: `python highlights.py AGENT_IDX HIGHLIGHT_REEL_IDX`
    """
    ...
    assert len(sys.argv) == 3, "Must specify the agent idx and highlight index through program arguments."
    ...
    highlight_idx_i = int(sys.argv[2])
    print(f"running highlight {highlight_idx_i}: {chosen_agent.highlight_reel.reelitem2dict(highlight_idx_i)}")
    if np.array_equal(highlights[highlight_idx_i], Agent.HighlightReel.EMPTY_ITEM):
        return

    prev_state = chosen_agent.highlight_reel.get_item_value(highlight_idx_i, "prev_state")
    next_state = chosen_agent.highlight_reel.get_item_value(highlight_idx_i, "next_state")
    ...
    # get trigger
    triggers = []
    trigger = int(chosen_agent.highlight_reel.get_item_value(highlight_idx_i, "trigger"))
    ...
    prev_beliefs = chosen_agent.highlight_reel.get_item_value(highlight_idx_i, "prev_beliefs")
    # delta from prev_beliefs to the new beliefs
    delta_beliefs = chosen_agent.highlight_reel.get_item_value(highlight_idx_i, "delta_beliefs")

    # get the time step
    time_step = chosen_agent.highlight_reel.get_item_value(highlight_idx_i, "time_step")
