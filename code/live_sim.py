...
class Sensor():
    def __init__(self, observer_loc):
        self.loc = coord2state(observer_loc, Singleton.instance.ncols)
...
class Simulation():
    ...
    def on_click(event):
        ...
        Singleton.instance.sensors.append(Sensor(tuple(reversed((floor(event.xdata), floor(event.ydata))))))
...
class Singleton():
    ...
    def __init__(self, ani, gwg, ax, agents, tr_ar, rl_ar, observable_regions, building_squares, nrows, ncols,
        ...
        if Singleton.instance is None:
            Singleton.instance = Simulation(ani, gwg, ax, agents, tr_ar, rl_ar, observable_regions, building_squares,
...
def grid_init(nrows, ncols, desiredIndices):
    ...
    for idx, id_no in enumerate(desiredIndices):
        ...
        currAgent = Agent(c_i=c_i, label=names[int(id_no[0])], char_name=id_no[0], \
                          bad_i=b_i, mdp=mdp_list[int(id_no[0])], state=state2prod(id_no[1], 0, state_keys), t_i=t_i,
...
def grid_update(i):
    ...
    if simulation.sensors is not None and simulation.ani.sensor_state != SensorState.DO_NOTHING:
        curr_sensor_loc = simulation.sensors[-1].loc
        if simulation.ani.sensor_state == SensorState.ADD_SENSOR:
            write_objects += simulation.add_observer(curr_sensor_loc)
        else:  # simulation.ani.sensor_state == SensorState.REMOVE_SENSOR:
            write_objects += simulation.remove_observer(curr_sensor_loc)
        simulation.ani.sensor_state = SensorState.DO_NOTHING
    else:
