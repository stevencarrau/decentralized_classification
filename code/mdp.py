...
class MDP(NFA):
    ...
    def sample(self, state, action):
        ...
        next_state = list(self.post(state, action))[
            np.random.choice(range(len(self.post(state, action))), 1, p=prob)[0]]
        ...
        while True:
            ...
            for s in self.states:
                if type(policy[s]) == set:
                    ...
                    a = policy[s]
                V1[s] = sum(
                    [self.prob_delta(s, a, next_s) * (gamma * V[next_s] + R[s, a]) for next_s in self.post(s, a)])
        ...
        for s in self.states:
            for a in self.available(s):
                for t in self.post(s, a):
                    file.write('{} {} {} {}\n'.format(list(self.states).index(s), a, list(self.states).index(t),
                                                      self.prob_delta(s, a, t)))
