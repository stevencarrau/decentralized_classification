...
def belief_chart_init():
	...
	# ax = plt.subplot(111, frame_on=True)
	# plt.tight_layout()
	plt.plot([0,110],[0,0],transform=ax.get_xaxis_transform(),linewidth=12,color='black',clip_on=False)
	...
	plt_array = []
	for i, id_no in enumerate([categories[0]]):
		belief_x_bad.append([])
...
def belief_update(i):
	...
	change_array = []
	for j, id_no in enumerate([categories[0]]):
...
for k_i in range(110):
	belief_update(k_i)
	plt.savefig('data/belief_charts/belief_chart_{0:03d}.png'.format(k_i), transparent=True)
# ax_ar = grid_init(gwg)
