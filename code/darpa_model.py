		...
		for a_i in t_list:
			for e_t in env_trans:
				for e_s in env_trans[e_t]:
					sub_trans = methods[e_t](e_s, t_list[a_i][e_s][0], copy.deepcopy(env_trans[e_t][e_s]), a_i, t_list[a_i][e_s][1])
					for s_b in sub_trans:
						for d_s in disrupt_trans:
							for d_t in disrupt_trans[d_s]:
								if len(disrupt_trans[d_s])==1:
									list_trans[idx] += [(product_reverse[(s_b[0],d_s)],s_b[1],product_reverse[(s_b[2],d_t)],perturbed_prob(s_b[3],deltas[d_s],1))]
								else:
									if d_s == d_t:
										list_trans[idx] += [(product_reverse[(s_b[0],d_s)],s_b[1],product_reverse[(s_b[2],d_t)],perturbed_prob(s_b[3],deltas[d_s],1-escalation_prob[a_i]))]
									else:
										list_trans[idx] += [(product_reverse[(s_b[0],d_s)],s_b[1],product_reverse[(s_b[2],d_t)],perturbed_prob(s_b[3],deltas[d_s],escalation_prob[a_i]))]
