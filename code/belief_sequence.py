...
fig = texfig.figure(width=3.3*scale_factor,ratio=3, dpi=my_dpi)
...
for row in range(0, len(df.index)):
	ax = plt.subplot(4, 1, row+1, polar=True)
	...
	# 	ax.add_artist(ab)
	l, = ax.plot([],[],color=my_palette(row),linewidth=2*scale_factor,linestyle='solid')
...
for s_i in range(0,1):
	update_all(s_i)
	texfig.savefig("images/Belief_simplex"+str(s_i))
